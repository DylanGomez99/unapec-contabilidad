...
public class AuxiliarController {
    ...
    @GetMapping
    @Operation(summary = "Listar todos los auxiliares")
    public List<Auxiliar> listarTodos() {
        return service.findAll();
    }

    @GetMapping("/{id}")
    @Operation(summary = "Obtener un auxiliar por ID")
    public ResponseEntity<Auxiliar> obtenerPorId(@PathVariable Long id) {
        return ResponseEntity.ok(service.getById(id));
    }

    @PostMapping
    @Operation(summary = "Crear un nuevo auxiliar")
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Actualizar un auxiliar existente")
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Eliminar un auxiliar")
